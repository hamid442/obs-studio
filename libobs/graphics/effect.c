...
gs_eparam_t *gs_technique_get_annotation_by_idx(const gs_technique_t *technique,
	size_t annotation)
{
	if (!technique) return NULL;
	struct gs_effect_param *params = technique->annotations.array;
	if (annotation > technique->annotations.num)
		return NULL;

	return params + annotation;
}

gs_eparam_t *gs_technique_get_annotation_by_name(const gs_technique_t *technique,
	const char *name)
{
	if (!technique) return NULL;
	struct gs_effect_param *params = technique->annotations.array;

	for (size_t i = 0; i < technique->annotations.num; i++) {
		struct gs_effect_param *g_param = params + i;
		if (strcmp(g_param->name, name) == 0)
			return g_param;
	}
...
gs_eparam_t *gs_pass_get_annotation_by_idx(const gs_epass_t *pass,
	size_t annotation)
{
	if (!pass) return NULL;
	struct gs_effect_param *params = pass->annotations.array;
	if (annotation > pass->annotations.num)
		return NULL;

	return params + annotation;
}

gs_eparam_t *gs_pass_get_annotation_by_name(const gs_epass_t *pass,
	const char *name)
{
	if (!pass) return NULL;
	struct gs_effect_param *params = pass->annotations.array;

	for (size_t i = 0; i < pass->annotations.num; i++) {
		struct gs_effect_param *g_param = params + i;
		if (strcmp(g_param->name, name) == 0)
			return g_param;
	}
