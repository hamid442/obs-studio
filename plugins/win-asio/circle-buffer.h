...
public:
	...
	bool render_audio(device_source_audio* asio_buffer, long route[])
	{
		...
		for (short i = 0; i < aoi.speakers; i++) {
			size_t total_chs = asio_buffer->input_chs + asio_buffer->output_chs;
			if (route[i] >= 0 && route[i] < total_chs) {
...
public:
	...
	*/
	...
	{
		prep_events(options.in_channel_count, options.out_channel_count);
	}

	void prep_events(long input_chs, long output_chs = 0)
	{
		if (!events_prepped) {
			device_options.in_channel_count  = input_chs;
			device_options.out_channel_count = output_chs;
			/*
			receive_signals = (WinHandle*)calloc(input_chs + output_chs, sizeof(WinHandle));
			for (int i = 0; i < input_chs + output_chs; i++) {
	...
	{
		prep_buffers(options.buffer_size, in_format, in_samples_per_sec, options.in_channel_count,
				options.out_channel_count);
	}

	void prep_buffers(uint32_t frames, audio_format format, uint32_t samples_per_sec, uint32_t in_chs,
			uint32_t out_chs = 0)
	{
		...
		prep_events(in_chs, out_chs);
		if (circle_buffer_prepped && (!buffer_prepped || reallocate_buffer)) {
			this->frames                     = frames;
			device_options.buffer_size       = frames;
			this->input_chs                  = in_chs;
			this->output_chs                 = out_chs;
			device_options.in_channel_count  = in_chs;
			device_options.out_channel_count = out_chs;
			this->format                     = format;
			...
			for (int i = 0; i < buffer_count; i++) {
				device_source_audio* _source_audio = get_source_audio(i);
				uint32_t             total_chs     = input_chs + output_chs;
				_source_audio->data                = (uint8_t**)bzalloc(total_chs * sizeof(uint8_t*));
				...
				_source_audio->output_chs      = output_chs;
	...
	void write_buffer_interleaved(const void* buffer, DWORD BufSize, uint64_t timestamp_on_callback)
	{
		...
		size_t   ch_buffer_size = BufSize / device_options.in_channel_count; // info.inputs;
		if (ch_buffer_size > buffer_size) {
			blog(LOG_WARNING, "%s device needs to reallocate memory");
		}
		int    byte_depth             = bytedepth_format(format);
		size_t interleaved_frame_size = device_options.in_channel_count * byte_depth; // info.inputs
		...
		for (size_t i = 0; i < frames_count; i++) {
			for (size_t j = 0; j < device_options.in_channel_count; j++) {
		...
		_source_audio->input_chs       = device_options.in_channel_count;
		_source_audio->samples_per_sec = samples_per_sec;
	...
	void write_buffer_planar(DWORD BufSize, DWORD OutBufSize, uint64_t timestamp_on_callback, const void* buffer,
			const void* buffer_2)
	{
		if (!all_prepped) {
			blog(LOG_INFO, "%s device %i is not prepared", __FUNCTION__, device_index);
		...
		uint8_t** input_buffer  = (uint8_t**)buffer;
		uint8_t** output_buffer = (uint8_t**)buffer_2;
		...
		audio_format planar_format = get_planar_format(format);
		// deinterleave directly into buffer (planar)
		size_t t = device_options.in_channel_count; // + device_options.out_channel_count;
		if (input_buffer) {
		...
		t = device_options.in_channel_count + device_options.out_channel_count;
		size_t i = 0;
		if (output_buffer) {
			for (size_t j = device_options.in_channel_count; j < t; j++) {
				memcpy(_source_audio->data[j], output_buffer[i], ch_buffer_size);
			}
		} else {
			for (size_t j = device_options.in_channel_count; j < t; j++) {
				memset(_source_audio->data[j], 0, ch_buffer_size);
	...
	static DWORD WINAPI capture_thread(void* data)
	{
		...
		thread_name += ":";
		thread_name += device->device_options
					       .name; // device->device_info.name;//thread_name += " capture thread";
		os_set_thread_name(thread_name.c_str());

		HANDLE signals_1[3] = {device->all_recieved_signal, device->stop_listening_signal,
				source->stop_listening_signal};
		HANDLE signals_2[3] = {device->all_recieved_signal_2, device->stop_listening_signal,
				source->stop_listening_signal};
		...
		while (source && device) {
			...
			if (waitResult == WAIT_OBJECT_0) {
				while (read_index != device->write_index) {
					device_source_audio* in = device->get_source_audio(
				...
				if (source->device_index != device->device_index) {
					blog(LOG_INFO, "source device index %lu is not device index %lu",
							source->device_index, device->device_index);
				...
				// os_sleepto_ns(os_gettime_ns() + ((device->frames * NSEC_PER_SEC) /
				// device->samples_per_sec)); Sleep(1); microsoft docs on the return codes gives the
				// impression that you're supposed to subtract wait_object_0
