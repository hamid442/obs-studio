...
OBS_DECLARE_MODULE()
OBS_MODULE_USE_DEFAULT_LOCALE("obs-slideshow", "en-US")
/* ------------------------------------------------------------------------- */
struct image_source {
	obs_source_t *source;
...
{
	struct slideshow *ss = static_cast<struct slideshow *>(data);
...
{
	struct slideshow *ss = static_cast<struct slideshow *>(bzalloc(sizeof(*ss)));
...
static obs_properties_t *ss_properties(void *data)
{
	...
	if (ss) {
		pthread_mutex_lock(&ss->mutex);
		if (ss->files.num) {
			struct image_file_data *last =
				static_cast<struct image_file_data *>(da_end(ss->files));
			const char *slash;
...
{
	struct obs_source_info slideshow_info = { 0 };
	slideshow_info.id = "slideshow";
	slideshow_info.type = OBS_SOURCE_TYPE_INPUT;
	slideshow_info.output_flags = OBS_SOURCE_VIDEO |
