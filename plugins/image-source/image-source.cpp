...
{
	struct image_source *context = static_cast<struct image_source *>(data);
	...
	if (context->persistent || obs_source_showing(context->source))
		image_source_load(static_cast<struct image_source *>(data));
	else
		image_source_unload(static_cast<struct image_source *>(data));
}
...
{
	struct image_source *context = (struct image_source *)data;
...
{
	struct image_source *context =
		(struct image_source *)bzalloc(sizeof(struct image_source));
...
{
	struct image_source *s = (struct image_source *)data;
	struct dstr path = {0};
...
{
	struct image_source *s = static_cast<struct image_source *>(data);
...
{
	struct obs_source_info image_source_info = { 0 };
	image_source_info.id = "image_source",
	image_source_info.type = OBS_SOURCE_TYPE_INPUT,
	image_source_info.output_flags = OBS_SOURCE_VIDEO,
