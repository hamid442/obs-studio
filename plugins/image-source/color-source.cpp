...
{
	struct color_source *context = static_cast<struct color_source *>(data);
...
static void *color_source_create(obs_data_t *settings, obs_source_t *source)
{
	...
	struct color_source *context =
		(struct color_source *)bzalloc(sizeof(struct color_source));
	context->src = source;
...
{
	struct obs_source_info color_source_info = { 0 };
	color_source_info.id = "color_source";
	color_source_info.type = OBS_SOURCE_TYPE_INPUT;
	color_source_info.output_flags = OBS_SOURCE_VIDEO | OBS_SOURCE_CUSTOM_DRAW;
	color_source_info.create = color_source_create;
	color_source_info.destroy = color_source_destroy;
	color_source_info.update = color_source_update;
	color_source_info.get_name = color_source_get_name;
	color_source_info.get_defaults = color_source_defaults;
	color_source_info.get_width = color_source_getwidth;
	color_source_info.get_height = color_source_getheight;
	color_source_info.video_render = color_source_render;
	color_source_info.get_properties = color_source_properties;
